...
* cperEvent
    ...
    CreateDump Method at /xyz/openbmc_project/dump/cper objPath with parameters
    below.
        - AdditionalData: a dict including key-value pairs below.
            * CPER_TYPE=cper or cperSection
            * CPER_PATH=/var/cper/cper-XXXXXXX

**note:** Cper.interface.yaml is proposed to be created for new CPER dump type at
phosphor-dbus-interface repo/phosphor-dbus-interfaces/yaml/xyz/openbmc_project/Dump/Entry/.
 The createDump method should move the CPER binary file from CPER_PATH to the
 dump folder managed by Dump.Manager and perform the action when the total
 number or size of CPER dumps exceeded the predefined limitation.
